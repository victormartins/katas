class RomanConverter {
  ...
  static convert(value) {
    const arabicToRoman = new ArabicToRoman({romanNumerals: this.ROMAN_NUMERALS})
    const romanToArabic = new RomanToArabic({romanNumerals: this.ROMAN_NUMERALS})
...
class ArabicToRoman{
  ...
  execute(arabic){
    ...
    while(remainder > 0){
      const romanValue = Object.entries(this.romanNumerals).find(r_v => r_v[1] <= remainder);
      const roman = romanValue[0];
      const value = romanValue[1];
...
class RomanToArabic{
  ...
  execute(roman){
    return Array.from(roman)
      ...
      .reduce((total, charIndex) => {
        const [char, index] = charIndex;
        const value = this.romanNumerals[char];
        const nextValue = this.romanNumerals[roman[index+1]]

        if(nextValue && nextValue > value) return total -= value;
        return total += value;
      }, 0)
  }
}
