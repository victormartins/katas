...
class RomanNumeralsConverter
  ...
  def convert(number)
    return arabic_to_roman.call(number) if number.is_a?(Numeric)
...
class ArabicToRoman
  def initialize(roman_to_arabic)
    @roman_numerals = roman_to_arabic
  end

  def call(arabic)
    raise(Error, 'Input must be Numeric') unless arabic.is_a?(Numeric)
...
class RomanToArabic
  ...
  def call(roman)
    raise(Error, 'Input must be a String') unless roman.is_a?(String)
