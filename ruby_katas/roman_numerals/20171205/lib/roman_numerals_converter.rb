...
class RomanNumeralsConverter
  ...
  def convert(number)
    return arabic_to_number(number) if number.is_a?(Integer)
  ...
  def char_to_number(char)
    ROMAN_NUMERALS.fetch(char) { 0 }
  end

  Contract Num => String
  def arabic_to_number(number)
    return convert_arabic_to_number(number) if number >= 0
    "-#{convert_arabic_to_number(-number)}"
  end

  Contract Num => String
  def convert_arabic_to_number(number)
